...
def save_results(data, file_path):
    """Overwrite the input JSON file with processed results."""
...
def parse_args():
    ...
    parser.add_argument("--input_json", type=str, help="Path to the input JSON dataset to overwrite")
...
def process_caption_scoring(data, instruction, model):
    ...
    for entry in tqdm(data, desc="Scoring captions"):
        ...
        caption = f"General caption: {entry.get('general_caption', '')}\nDetail caption: {entry.get('task_caption', '')}"
...
def main():
    ...
    if instruction is None:
        print(f"Task '{args.task}' not found in instruction file.")
        sys.exit(1)
    ...
    save_results(processed_data, args.input_json)
    print(f"Processing completed. Input file '{args.input_json}' overwritten with results.")
